...
func (g *Gofer) setupMounts(conf *config.Config, mounts []specs.Mount, root, procPath string, rpcClnt *urpc.Client) error {
	...
	for _, m := range mounts {
		...
		if err := rpcClnt.Call("goferRPC.OpenMount", &container.OpenMountArgs{Path: m.Source}, &res); err != nil {
			return fmt.Errorf("opening %s: %w", m.Source, err)
		}
		defer res.Files[0].Close()
		src := fmt.Sprintf("%s/self/fd/%d", procPath, res.Files[0].Fd())
		if err := specutils.SafeSetupAndMount(src, dst, m.Type, flags, procPath); err != nil {
