...
func (c *CPU) SwitchToUser(switchOpts SwitchOpts) (vector Vector) {
	...
	WriteFS(uintptr(ptRegs.Fs_base))                   // escapes: no. Set application FS.
	WriteGS(uintptr(ptRegs.Gs_base))                   // escapes: no. Set application GS.
	...
	WriteFS(uintptr(c.registers.PtraceRegs().Fs_base))            // escapes: no. Restore kernel FS.
	return
}
